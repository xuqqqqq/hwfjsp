...
def candidate_policy_library(features: dict[str, Any], track: str, target_setup: int) -> list[dict[str, Any]]:
    ...
    candidates = [
        ...
        candidate_from(
            base,
            candidate_id="lowsetup_rulehook_density_push_relax1",
            label="低切换密度推进模板-relax1",
            updates={
                "lookahead": 90,
                "start_guard": 120,
                "score_density": 22500.0,
                ...
                "score_progress": 120.0,
                "phase2_progress": 80.0,
                "score_zero_setup": 490.0,
                "phase2_zero_setup": 410.0,
                "score_setup_fixed": 490.0,
                "phase2_setup_fixed": 600.0,
                "score_setup_per": 4.1,
                "phase2_setup_per": 4.6,
                "batch_group_wait": 300 if finite else 0,
                "batch_group_mixed_time": True if finite else False,
                "phase2_hook_density": 76.0,
                "phase2_urgency_threshold": 0.65,
                "phase2_urgency_bonus": 430.0,
                "phase2_upper_bound_bonus": 220.0,
                "phase2_batch_bonus": 140.0,
            },
            reasons=["在 18484/498 模板上极小幅放松 setup 惩罚，尝试用不超过 2 次切换换取 16 以上产量。"],
            prior_reward=18410.0,
            score_hook_template="lowsetup_load_balance",
            added_rules=[
                "沿用低切换 score_path 与 phase1 负载均衡惩罚。",
                "仅放松 setup 惩罚强度，不改变 lookahead、组批等待和主要密度结构。",
            ],
            removed_rules=[
                "继续不使用 phase1 组批潜力额外奖励。",
            ],
        ...
        candidate_from(
            base,
            candidate_id="lowsetup_rulehook_density_push_urgent1",
            label="低切换密度推进模板-urgent1",
            updates={
                "lookahead": 90,
                "start_guard": 120,
                "score_density": 22500.0,
                ...
                "batch_group_mixed_time": True if finite else False,
                "phase2_hook_density": 76.0,
                "phase2_urgency_threshold": 0.6,
                "phase2_urgency_bonus": 520.0,
                "phase2_upper_bound_bonus": 260.0,
                "phase2_batch_bonus": 140.0,
            },
            reasons=["保持 setup 约束强度不变，仅强化二阶段临界任务和可完工上界保护。"],
            prior_reward=18408.0,
            score_hook_template="lowsetup_load_balance",
            added_rules=[
                "沿用低切换 score_path 与 phase1 负载均衡惩罚。",
                "phase2 对临界窗口任务增加紧迫度和上界保护，优先补齐可能压线完工的工件。",
            ],
            removed_rules=[
                "继续不使用 phase1 组批潜力额外奖励。",
            ],
            kept_rules=[
                "保留低切换规则模板的路径评分、同族连续、零 setup 与 setup 抑制。",
            ],
        ),
        candidate_from(
            base,
            candidate_id="lowsetup_rulehook_density_push_density_micro",
            label="低切换密度推进模板-density-micro",
            updates={
                "lookahead": 90,
                "start_guard": 120,
                "score_density": 22500.0,
                "phase2_density": 6775.0,
                "score_family": 700.0,
                "phase2_family": 800.0,
                "score_progress": 120.0,
                "phase2_progress": 80.0,
                ...
                "batch_group_wait": 300 if finite else 0,
                "batch_group_mixed_time": True if finite else False,
                "phase2_hook_density": 76.3,
                "phase2_urgency_threshold": 0.65,
                "phase2_urgency_bonus": 420.0,
                "phase2_upper_bound_bonus": 220.0,
                "phase2_batch_bonus": 140.0,
            },
            reasons=["只做二阶段密度的微小推进，验证 v1b 失败是否来自 phase1 密度扰动。"],
            prior_reward=18407.0,
            score_hook_template="lowsetup_load_balance",
            added_rules=[
                "沿用低切换 score_path 与 phase1 负载均衡惩罚。",
                "仅提高 phase2 密度和 hook 密度，不动 phase1 密度与组批等待。",
            ],
            removed_rules=[
                "继续不使用 phase1 组批潜力额外奖励。",
            ],
        ...
        candidate_from(
            base,
            candidate_id="lowsetup_density_push_lookahead95",
            label="低切换密度推进模板-lookahead95",
            updates={
                "lookahead": 95,
                "start_guard": 120,
                "score_density": 22500.0,
            ...
            reasons=["只扩大 lookahead，不改评分结构，验证候选视野是否限制了 500 setup 边界。"],
            prior_reward=18409.0,
            score_hook_template="lowsetup_load_balance",
            added_rules=[
                "phase1/phase2 维持低切换评分，只扩大候选观察窗口。",
            ],
            removed_rules=[
                "不引入 task_bonus 或 slack-margin 规则。",
            ],
            kept_rules=[
                "保留当前最优低切换模板的全部评分权重。",
            ],
        ...
        candidate_from(
            base,
            candidate_id="lowsetup_density_push_phase2_relax_micro",
            label="低切换密度推进模板-phase2-relax-micro",
            updates={
                "lookahead": 90,
                "start_guard": 120,
                "score_density": 22500.0,
                "phase2_density": 6825.0,
                "score_family": 700.0,
                "phase2_family": 790.0,
                "score_progress": 120.0,
                "phase2_progress": 80.0,
                "score_zero_setup": 500.0,
                "phase2_zero_setup": 400.0,
                "score_setup_fixed": 500.0,
                "phase2_setup_fixed": 590.0,
                "score_setup_per": 4.2,
                "phase2_setup_per": 4.55,
                "batch_group_wait": 300 if finite else 0,
                "batch_group_mixed_time": True if finite else False,
                "phase2_hook_density": 77.0,
                "phase2_urgency_threshold": 0.65,
            ...
            reasons=[
                "当前 18484/498 距 18500 只差一个中等权重任务，尝试仅在二阶段微放松 setup 抑制。",
            ],
            prior_reward=18409.0,
            score_hook_template="lowsetup_load_balance",
            added_rules=[
                "二阶段略提高密度和 hook 密度，略降低 setup 惩罚。",
            ],
            removed_rules=[
                "一阶段保持当前最优模板，避免早期主序列失稳。",
            ],
            kept_rules=[
                "保留同族连续和零 setup 奖励，不复制高 setup 桥接解。",
            ],
        ),
        candidate_from(
            base,
            candidate_id="lowsetup_density_push_family_guard_micro",
            label="低切换密度推进模板-family-guard-micro",
            updates={
                "lookahead": 90,
                "start_guard": 120,
                "score_density": 22600.0,
                ...
                "phase2_zero_setup": 450.0,
                "score_setup_fixed": 510.0,
                "phase2_setup_fixed": 630.0,
                "score_setup_per": 4.25,
                "phase2_setup_per": 4.85,
                "batch_group_wait": 300 if finite else 0,
                "batch_group_mixed_time": True if finite else False,
                "phase2_hook_density": 77.0,
                "phase2_urgency_threshold": 0.65,
            ...
            reasons=[
                "提高密度推进的同时加强同族和零 setup 防守，测试能否获得少量产量而 setup 不越界。",
            ],
            prior_reward=18409.0,
            score_hook_template="lowsetup_load_balance",
            added_rules=[
                "密度推进与 family/zero-setup 防守同时微调。",
            ],
            removed_rules=[
                "不使用任务级 bonus 和候选池接管。",
            ],
            kept_rules=[
                "保留低切换 score_path 和二阶段紧迫保护。",
            ],
        ),
        candidate_from(
            base,
